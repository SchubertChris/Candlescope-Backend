...
router.post('/templates', requireAuth, requireAdmin, async (req, res) => {
  try {
    const { name, subject, preheader, content, images, scheduledDate, templateCategory } = req.body;
...
router.post('/templates/:id/preview', requireAuth, requireAdmin, async (req, res) => {
  try {
    const template = await NewsletterTemplate.findById(req.params.id);
...
router.post('/templates/:id/send', requireAuth, requireAdmin, async (req, res) => {
  try {
    const template = await NewsletterTemplate.findById(req.params.id);
...
router.get('/stats', requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    if (recentNewsletters.length > 0) {
      const totalSent = recentNewsletters.reduce((sum, nl) => sum + nl.sentCount, 0);
      const totalOpened = recentNewsletters.reduce((sum, nl) => sum + nl.openedCount, 0);
...
router.post('/subscribe', async (req, res) => {
  try {
    const { email, firstName, lastName, source = 'newsletter_signup' } = req.body;
    ...
    res.status(201).json({
      success: true,
      message: 'Bitte überprüfen Sie Ihre E-Mails und bestätigen Sie Ihre Anmeldung',
