...
app.get("/", (req, res) => {
  const baseURL =
    process.env.NODE_ENV === "production" ? `https://candlescope-backend.onrender.com` : `http://localhost:${PORT}`;
...
const startServer = async () => {
  try {
    ...
    const server = app.listen(PORT, "0.0.0.0", () => {
      ...
      console.log(
        `👑 Admin Account: ${
          process.env.ADMIN_EMAIL && process.env.ADMIN_PASSWORD ? "Auto-Creation ✅" : "Manual Setup Required ❌"
        }`
      );
